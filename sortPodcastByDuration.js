...
function sortByDuration(data, flightLength) {
  //   check if flight is longer than 60 min
  if (flightLength > 60) {
    // sort function descending order
    data.sort((a, b) => b - a);
    data.forEach(({ title, duration }) =>
      console.log(`${title} ${duration} minutes`)
    );
  } else {
    //  if light is shorter than 60
    // sort function in ascending order
    data.sort((a, b) => a - b);
    // loop through and console.log(title, time)
    data.forEach(({ title, duration }) =>
      console.log(`${title} ${duration} minutes`)
    );
  }
